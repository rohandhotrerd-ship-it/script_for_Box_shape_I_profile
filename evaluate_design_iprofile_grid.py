...
evaluate_design_iprofile_grid.py

Runs ONE design through the existing MASTER_GH_TO_ABAQUS3_iprofile_grid.py pipeline.
...
# - MASTER_GH_TO_ABAQUS3_iprofile_grid.py must support the new "coords" field for the
...
# PATH CONFIG
# ============================================================
MASTER_SCRIPT = r"C:\Rhino Hiwi\Thesis\cad_to_stp\BoxShape_Iprofile\iprofile_grid_one_stage_scripts\MASTER_GH_TO_ABAQUS3_iprofile_grid.py"
PYTHON_EXE = sys.executable

WORK_DIR = r"C:\Rhino Hiwi\Thesis\cad_to_stp\BoxShape_Iprofile\iprofile_grid_one_stage_scripts\Iprofile_gird_GA_work"
...
REFERENCE_MODEL_NAME = "IPROFILE_GRID_REFERENCE_TO_BE_DEFINED"
...
def sanitize_design(design):
    """
    One-stage row-column I-profile grid design dictionary.
    ...
    d["Width"] = _safe_float(d.get("Width", 6.0), "Width")
    ...
    if d["Width"] < 1.0 or d["Width"] > 20.0:
        raise RuntimeError("Width out of range [1, 20]: %s" % d["Width"])
...
def _make_design_tag(design):
    """
    Compact readable tag for the fixed row-column I-profile grid study.
    """
    return "GRID_H%s_W%s_TS%s_TW%s_SM%s" % (
        ("%.2f" % design["Height"]).replace(".", "_"),
        ("%.2f" % design["Width"]).replace(".", "_"),
...
def _extract_run_folder_from_stdout(stdout_text):
    ...
    # Format used by MASTER_GH_TO_ABAQUS3_iprofile_grid.py final summary.
...
def _extract_failure_stage_and_error(stdout_text, stderr_text):
    ...
    if failure_stage is None:
        ...
        for name in [
            ...
            "Mesh_testing2.py",
            "00_import_parts_makeprecise_assemble3.py",
            "04_property_step_interaction_load_bc3.py",
            "05_job_write_datacheck_submit3.py",
            "06_postprocess_dat2.py",
            "RUN_ALL_PIPELINE3.py",
            "MASTER_GH_TO_ABAQUS3_iprofile_grid.py",
        ]:
            if name in text:
                failure_stage = name
                break
...
def _compute_lambda_1_crit(summary_data):
    """
    Fitness-driving lambda.

    New rule:
    # - use ONLY the first positive eigenvalue from the Combined case
    # - Compression and Shear values are still kept for CSV/debugging,
    #   but they do NOT control the fitness value anymore.
    """
    if not isinstance(summary_data, dict):
        return None

    val = summary_data.get("lambda_first_positive_combined")
    if val is None:
        return None
...
def _soft_lambda_penalty(lambda_for_fitness):
    """
    Soft quadratic penalty used in BOTH stage-1 and stage-2.

    New rule:
    # - penalty is based ONLY on the Combined first-positive lambda
    # - Compression and Shear are not used for fitness/penalty anymore.
    """
    if lambda_for_fitness is None:
        return 0.0

    try:
        v = float(lambda_for_fitness)
    except:
        return 0.0

    if v < LAMBDA_LIMIT:
        return LAMBDA_PENALTY_K * ((LAMBDA_LIMIT - v) ** 2)

    return 0.0


def _hard_stage2_lambda_penalty(stage_name, lambda_for_fitness):
    """
    Hard penalty is applied ONLY in stage-2.

    New rule:
    # - stage-2 hard penalty checks ONLY the Combined first-positive lambda.
    """
...
def _compute_fitness_paper(lambda_1_crit, mass, lambda_comp=None, lambda_shear=None, lambda_combined=None, stage_name=None):
    ...
    # - hard +100 penalty ONLY in stage-2

    IMPORTANT:
    # lambda_1_crit is now the Combined-case first positive eigenvalue.
    # Compression and Shear lambdas are accepted only for backward-compatible
    # function calls; they are not used in the fitness calculation.
    """
    if lambda_1_crit is None or mass is None or mass <= 0.0:
    ...
    # --------------------------------------------------------
    soft_penalty = _soft_lambda_penalty(lambda_1_crit)

    # --------------------------------------------------------
    # Hard penalty is applied ONLY in stage-2.
    # --------------------------------------------------------
    hard_penalty = _hard_stage2_lambda_penalty(stage_name, lambda_1_crit)
...
def evaluate_design(design, keep_temp_files=True, verbose=True):
    ...
    try:
        ...
        result = {
            ...
            # First positive eigenvalues from each load case.
            # The GA fitness uses lambda_first_positive_combined only.
            "lambda_first_positive_compression": None if not isinstance(summary_data, dict) else summary_data.get("lambda_first_positive_compression"),
            "lambda_first_positive_shear": None if not isinstance(summary_data, dict) else summary_data.get("lambda_first_positive_shear"),
            "lambda_first_positive_combined": None if not isinstance(summary_data, dict) else summary_data.get("lambda_first_positive_combined"),
        ...
        return {
            ...
            "lambda_first_positive_compression": None,
            "lambda_first_positive_shear": None,
            "lambda_first_positive_combined": None,
            "case_dir": None,
            "summary_json": None,
            "summary_data": None,
            "stdout_log": stdout_log_path,
