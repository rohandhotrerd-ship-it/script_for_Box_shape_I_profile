...
GH_FILE = r"C:\Rhino Hiwi\new\Cluster for Y-line\New folder\Recovered_files\BoxShape_I-profile.ghx" #r"C:\Rhino Hiwi\new\Cluster for Y-line\Thesis_Voronoi-Iprofile.ghx"
BASE_DIR = r"C:\Rhino Hiwi\Thesis\cad_to_stp\BoxShape_Iprofile\GABox_Iprofile_for_m0.6_L0.4_coords10"
DESIGN_INPUT_LIVE = r"C:\Rhino Hiwi\Thesis\cad_to_stp\BoxShape_Iprofile\design_input_live.json"

ABAQUS_BAT = r"C:\SIMULIA\abaqus\Commands\abaqus.bat"
ABAQUS_PIPELINE_SCRIPT = r"C:\Rhino Hiwi\Thesis\cad_to_stp\BoxShape_Iprofile\iprofile_grid_one_stage_scripts\RUN_ALL_PIPELINE_Iprofile_grid.py"

ABAQUS_WORKING_DIR = r"C:\Rhino Hiwi\Thesis\cad_to_stp\BoxShape_Iprofile\iprofile_grid_one_stage_scripts\working_dir"
...
DEFAULT_DESIGN = {
    "coords":list(DEFAULT_COORDS), #[0.104549, 0.034582, 0.187348, 0.058939, 0.309575, 0.053279, 0.45212, 0.057797, 0.656282, 0.066483, 0.764422, 0.050951, 0.892037, 0.123665, 0.097764, 0.510913, 0.180537, 0.522913, 0.319608, 0.487946, 0.558743, 0.514, 0.646834, 0.518461, 0.821142, 0.5276, 0.887486, 0.45321, 0.057854, 0.896774, 0.213318, 0.964291, 0.461164, 0.901468, 0.602653, 0.909563, 0.801746, 0.915885, 0.891786, 0.894663],
    "Height": 20.0,
    "Width": 4.0,
    "Thickness_skin": 3.0,
    "Thickness_stiff": 3.0,
...
def check_paths():
    ...
    if not os.path.isfile(ABAQUS_PIPELINE_SCRIPT):
        raise RuntimeError("RUN_ALL_PIPELINE3.py not found:\n%s" % ABAQUS_PIPELINE_SCRIPT)
...
def _safe_coords_list(v):
    # Grid row-column model:
    # coords are optional because the topology is fixed in Grasshopper.
    if v is None:
        return []
    ...
    for idx, item in enumerate(v):
        ...
        # Keep normalized bounds identical to old Gene Pool logic
        if val < 0.0:
            val = 0.0
        if val > 1.0:
            val = 1.0
    ...
    # For grid model, empty coords are allowed.
    if len(out) == 0:
        return []

    # If coords are provided, they must still be valid x,y pairs.
    if (len(out) % 2) != 0:
...
def _bounded(design):
    ...
    d["Width"] = _safe_float(d["Width"], "Width")
    ...
    if d["Width"] < 1.0 or d["Width"] > 10.0:
        raise RuntimeError("Width out of range [1, 10]: %s" % d["Width"])
    ...
    if d["stud_max"] < 12.0 or d["stud_max"] > 30.0:
...
def load_external_design_from_argv():
    ...
    Expected usage from evaluate_design.py:
        python MASTER_GH_TO_ABAQUS3_iprofile_grid.py <design_input_json>
    """
    if len(sys.argv) < 2:
...
def write_design_input_live_json(design, source_json_path=None):
    ...
    payload = {
        ...
        "written_by": "MASTER_GH_TO_ABAQUS3_live_json.py",
...
def apply_design_to_gh_inputs(design):
    ...
    - Height / Width / stud_max are still pushed because the
      GH model may still use those values directly in other branches
    """
    GH_INPUTS["RH_IN:Height"] = _round2(design["Height"])
    GH_INPUTS["RH_IN:Width"] = _round2(design["Width"])
...
def write_design_vars_json(run_folder, design, source_json_path=None):
    ...
    payload = {
        ...
        "run_baseline": design.get("run_baseline"),
        "source_json_path": source_json_path,
        "written_by": "MASTER_GH_TO_ABAQUS3_minimal_coords.py",
        "timestamp_epoch_sec": time.time(),
    }
...
def run_grasshopper_export():
    ...
    file_type = GH_INPUTS["RH_IN:File_Type"]
...
def print_final_summary(gh_info, abaqus_info, results_info, design_info):
    ...
    print("  Width          : %s" % design_info.get("Width"))
